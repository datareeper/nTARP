...
vignette: >
  %\VignetteIndexEntry{nTARP Documentation}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

# 1. Introduction to the n-TARP clustering method
This R Notebook documents the functionality of the nTARP package. At its core, the package implements a high-dimensional clustering technique called Thresholding After Random Projections (n-TARP). In many datasets, the number of variables (dimensionality) can be much larger than the number of observations, which can make traditional distance-based clustering approaches unreliable or prone to non-convergence. To address this, n-TARP projects the high-dimensional data (in \( D \)) into one-dimensional spaces (in \( D' \)) repeatedly and applies k-means in each projection to identify clusters (typically splitting the data into two clusters at each step). 
...
![](../figures/nTARP_illustration.png)

The algorithm flows like so:

Let \( x_1, \dots, x_m \in \mathbb{R}^p \) denote the observations in the dataset, \( D \). And initialize lists, \( C \) (to store individual candidate cluster solutions), \( M \) (to store the cluster membership for each \( x_i \)), and \( W_L \) (to store the within sum of squares for each candidate solution).
...
1. For each projection, \( i = 1, \dots, n \):
   ...
   d. Compute the normalized within-cluster sum of squares \( W_i \) for this clustering and add it to the list \( W_L \).
...
Let's install the package and load it.
